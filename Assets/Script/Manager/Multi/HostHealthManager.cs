...
public class HostHealthManager : MonoBehaviourPun
{
    ...
    private void Start()
    {
        ...
        _view = photonManager.GetComponent<PhotonView>();
    }

    public void MultiAttack(float dmg)
    {
        _view.RPC("Attack",RpcTarget.All,dmg);
    }

    [PunRPC]
    public void Attack(float dmg)   // Host is Player, Guest is Enemy
    {
        if (dmg > 0)
        {
            EnemyHealth -= dmg;
